...
CONTAINS
  ...
  SUBROUTINE CLOSE_ALL(DATA,DATA_N)

    IMPLICIT NONE

    TYPE(DATA_IN), INTENT(IN) :: DATA , DATA_N
    INTEGER :: ierr
    INTEGER :: ii
    INTEGER :: NUM
    !CHARACTER (len=30) :: dir
...
!!$          end if
          ...
          !V%topoVF(i) = -1.0d0 + (1.0d0*tanh(3.0d01*(x - 0.0d-01))/4.0d0) !-1.0d0 !(- 1.25d0 - 0.75)/2.0d0 + (1.0d0*tanh(3.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0) ! ! !-1.0d0
          
          V%VF(i)%h = 1.0d0  !0.3d0*exp(-((MESH%X_DF(i)+0.5d0)/1.0d-01)**2) + 1.0d0 - (1.0d0*tanh(3.0d01*(x - 0.0d-01))/4.0d0) !1.0d0 !1.0d0 + 0.3d0*exp(-((MESH%X_DF(i)+0.5d0)/1.0d-01)**2) !1.0d0 !2.0d0 !0.0d0 !2.0d0 !max(V%topoVF(i),0.15) !1.0d0
          V%VF(i)%qx = 0.0D0
          if( MESH%X_DF(i) < 0.6d0 .and. MESH%X_DF(i) >  0.4d0 ) then 
             V%topoVF(i) = (-10.0d0*(MESH%X_DF(i) - 0.6d0)*(MESH%X_DF(i) - 0.4d0)) !0.3d0*exp(-((MESH%X_DF(i)-0.5d0)/1.0d-01)**2) !(-30.0d0*(x - 0.4d0)*(x - 0.6d0))  !-1.0d0 !-1.0d0
          else
             V%topoVF(i) = 0.0d0 !0.3d0*exp(-((MESH%X_DF(i)-0.5d0)/1.0d-01)**2) !0.0d0
          end if
