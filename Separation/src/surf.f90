PROGRAM SURF1D
  ...
  DATA_N = DATA
  DATA_N%x_in = -DATA%x_out
  DATA_N%x_out = - DATA%x_in
  Run_N = Run
  ...
  call ALLOCATE_ALL(VAR_N , FLU_N ,MESH_N, DATA_N , RK_N)
  ...
  MESH_N%X_DF(-2:Nbx+4) = - MESH%X_DF(Nbx+4:-2)
  ...
  !CALL INITIAL_CONDITION(MESH_N,DATA_N,VAR_N,debug)



  !open(unit = 12 , file = 'Debit_droite.res' , status = 'new')

  !WRITE(6,*) 'Main : output of initial solution ...'

  N0 = 0
  CALL OUTPUT(N0, MESH, Var, DATA, Run, 'SOL1D.')
  !write(12,*) 0.0d0 , Var%VF(Nbx+1)%qx , 0.2d0*w*sin(w*0.0d0)
  !print*,0.0d0 , Var%VF(Nbx+1)%qx , 0.2d0*w*sin(w*0.0d0)
  !CALL OUTPUT(N0, MESH_N, VAR_N, DATA_N, Run_N, 'SOLND.')
  ...
  loop__main : DO Niter = 1, DATA%NtMax !-- [MAIN TIME EVLUTION LOOP -- BEGIN] 
     ...
     !CALL time_loop(VAR_N, FLU_N, MESH_N, DATA_N, Run_N)


     ! ### INCREMENTATION DU TEMPS TOTAL ###

     Run%time = Run%time + Run%Dt
     Run_N%time = Run_N%time + Run_N%Dt
     ...
     IF(Run%time >= DATA%TimeOut) THEN
        ...
        !write(12,*) Run%time , Var%VF(Nbx+1)%qx , 0.2d0*w*sin(w*Run%time)
        !print*,Run%time , Var%VF(Nbx+1)%qx , 0.2d0*w*sin(w*Run%time)
