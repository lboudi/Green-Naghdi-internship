PROGRAM SURF1D
  ...
  REAL(KIND=8) :: tol
  real*8 :: G_xin , G_xout
  integer :: proc_fichier , parallelisme
  ...
  CALL Initialise(i_case, DATA, Run , Me,G_xin,G_xout,Np)
  ...
  loop__main : DO Niter = 1, DATA%NtMax !-- [MAIN TIME EVLUTION LOOP -- BEGIN] 
     ...
     CALL time_loop(Var, FLU, MESH, DATA, Run,Me,Np)
