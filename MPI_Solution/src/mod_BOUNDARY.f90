...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                !TRAITEMENT BORD GAUCHE ETAT INTERMEDIAIRE
!!$                allocate(te(N) , yy(N))
!!$
!!$                do i = 1 , N
!!$                   te(i) = MESH%X_DF(i+1)
...
!!$                Tab(-2) = Tab(-1)
!!$
!!$                !TRAITEMENT BORD DROIT ETAT INTERMEDIAIRE
!!$
!!$                allocate(te(N) , yy(N))
!!$
!!$                do i = 1 , N
...
!!$                   yy(i) = (Tab(Nbx-i+1)%u) + 2.0d0*sqrt(g_r__8*Tab(Nbx-i+1)%h) 
!!$                end do
!!$                C4 =  interpolation(x_R,te,yy,N)
!!$
!!$                deallocate(te)
!!$                deallocate(yy)
!!$
!!$                call MPI_SEND(C4 , 1 , MPI_DOUBLE_PRECISION , Me + 1 , 10 , MPI_COMM_WORLD, statinfo)
!!$                call MPI_RECV(C3 , 1 , MPI_DOUBLE_PRECISION , Me + 1 , 11 , MPI_COMM_WORLD,status, statinfo)
!!$                Tab(Nbx+1)%u = ((C3 + C4)/(2.0d0))
!!$                Tab(Nbx+1)%qx = Tab(Nbx+1)%h*Tab(Nbx+1)%u
!!$
!!$                Tab(Nbx+1)%h = (((C3 - C4)**2)/(16.0d0*g_r__8))
!!$
!!$                Tab(Nbx+2) = Tab(Nbx+1)
!!$                Tab(Nbx+3) = Tab(Nbx+2)
!!$
