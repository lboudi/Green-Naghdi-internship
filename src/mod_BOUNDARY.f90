...
CONTAINS
  ...
  SUBROUTINE Boundary_VF(Tab, TabEx, topoVF, MESH, DATA, Run, ii)      
    ...
    case(1996)
       ...
       Tab(0)%qx = 4.42d0
       Tab(0)%h = 1.0d0 + 3.0d-01*(cos((8/(1.0d0*5.0d-02))*atan(1.0d0)*(time)))
       !CONDITION GAUCHE
...
!!$          yy(i) = (Tab(i+1)%u) - sqrt(g_r__8*Tab(i+1)%h)
          te(i) = MESH%X_DF(i)
          yy(i) = (Tab(i)%u) - sqrt(g_r__8*Tab(i)%h)
...
!!$          yy(i) = (Tab(i+1)%u) - 2.0d0*sqrt(g_r__8*Tab(i+1)%h)
          ...
          te(i) = MESH%X_DF(i)
          yy(i) = (Tab(i)%u) - 2.0d0*sqrt(g_r__8*Tab(i)%h)
       end do
       ...
       Tab(0)%h = newton_droite(Tab(0)%h , Tab(0)%u , C1)
       !Tab(0)%u = (C1 + 2.0d0*sqrt(g_r__8*Tab(1)%h))/2.0d0

       !Tab(0)%qx = (Tab(0)%h)*(Tab(0)%u)
       Tab(0)%u = Tab(0)%qx/Tab(0)%h
       Tab(-1)%h = Tab(0)%h
       Tab(-2)%h = Tab(-1)%h
       ...
       Tab(Nbx+1)%h = 2.0d0 !1.0d0 !1.0d0 + 3.0d-01*(cos((8/(1.0d0*5.0d-02))*atan(1.0d0)*(time))) !2.0d0
...
!!!!!Tab(Nbx+1)%h = newton_gauche(Tab(Nbx+1)%h , Tab(Nbx+1)%uu , interpolation(x_R,te,yy,N))
       Tab(Nbx+1)%u = interpolation(x_R,te,yy,N) - 2.0d0*sqrt(g_r__8*Tab(Nbx+1)%h)
       Tab(Nbx+1)%qx = (Tab(Nbx+1)%h)*(Tab(Nbx+1)%u)
       Tab(Nbx+2)%u = Tab(Nbx+1)%u
       Tab(Nbx + 3)%u = Tab(Nbx+2)%u
