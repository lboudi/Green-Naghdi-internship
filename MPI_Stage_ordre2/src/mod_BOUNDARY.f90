...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       if(Np /= 1) then 
          ...
          if(Me == 0) then
             ...
             !hA = (((C2 - C1)**2)/(16.0d0*g_r__8))
             
!!$             allocate(te(N) , yy(N))
!!$
!!$             do i = 1 , N
!!$                te(i) = MESH%X_DF(Nbx-i+1)
!!$                yy(i) = (Tab(Nbx-i+1)%u) + sqrt(g_r__8*Tab(Nbx-i+1)%h) + uA + sqrt(g_r__8*hA)
!!$             end do
!!$
!!$             x_R = zero_fonction(te,yy,N, Data%x_out , Data%x_in, Dt/4.0d0)
!!$
!!$             deallocate(te)
!!$             deallocate(yy)
!!$
...
!!$             deallocate(yy)
             ...
             call MPI_RECV(C_COMMUN , 1 , MPI_DOUBLE_PRECISION , Me + 1 , 15 , MPI_COMM_WORLD,status, statinfo)
             C1 = C_COMMUN
             ...
             N = 2
               ...
               Tab(Nbx + 1)%u = Tab(Nbx+1)%qx/Tab(Nbx+1)%h
                ...
                do i = 1 , N
                   te(i) = MESH%X_DF(i+1)
                   yy(i) = (Tab(i+1)%u) - sqrt(g_r__8*Tab(i+1)%h) + uA - sqrt(g_r__8*hA)
                end do

                x_L = zero_fonction(te,yy,N, Data%x_in , Data%x_out, Dt/4.0d0)
                ...
                C1 = interpolation(x_L,te,yy,N)
                C_COMMUN = C1
                call MPI_SEND(C_COMMUN , 1 , MPI_DOUBLE_PRECISION , Me - 1 , 15 , MPI_COMM_WORLD, statinfo)
