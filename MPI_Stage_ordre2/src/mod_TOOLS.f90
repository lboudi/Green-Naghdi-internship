...
99998 CONTINUE
  ...
  SUBROUTINE Initialise(i_case, DATA, Run, ProcId,G_xin,G_xout,Np)
    ...
    case(1996)
       ...
       DATA%x_in = G_xin + ProcId*(abs(G_xout - G_xin)/Np) !0.0D0
       DATA%x_out = G_xin + (ProcId+1)*(abs(G_xout - G_xin)/Np)

       print*,"ProcId = ","   ",ProcId,"   ","DATA_xin = ","  ",DATA%x_in,"   ","DATA_xout = ","    ",DATA%x_out
       DATA%NtMax =  2000000 ! 90000000
       DATA%TimeInit = 0.0D0

       DATA%TimeOut = 20.0D0 ! 900.0d0
       DATA%limiter = 1
