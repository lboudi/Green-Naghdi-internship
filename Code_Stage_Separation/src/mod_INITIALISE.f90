    ...
    case(1996)
       ...
       do i = -2 , Nbx + 3
          if( MESH%X_DF(i) < 0.6d0 .and. MESH%X_DF(i) >  0.4d0 ) then         
             V%topoVF(i) =  (-10.0d0*(MESH%X_DF(i) - 0.4d0)*(MESH%X_DF(i) - 0.6d0)) !0.2d0 - 0.05d0*(MESH%X_DF(i) - 10)**2
          else
             V%topoVF(i) = 0.0d0            
          end if
          ...
          V%VF(i)%h = 1.0d0 !2.0d0 !!1.0d0 + 0.3d0*exp(-((MESH%X_DF(i)-0.5d0)/1.0d-01)**2)-  V%topoVF(i) !1.0d0 !2.0d0 !0.0d0 !2.0d0 !max(V%topoVF(i),0.15) 
          V%VF(i)%qx = 0.0D0
