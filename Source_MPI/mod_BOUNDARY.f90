...
CONTAINS
  ...
  SUBROUTINE Boundary_VF(Tab, TabEx, topoVF, MESH, DATA, Run, ii)      
    ...
    case(1996)
       ...
       !Tab(-2:Nbx+3)%h = 0.5d0 - Tab(-2:Nbx+3)%qx
...
!!!!U(1)%uu = C1 + 2.0d0*sqrt(g*U(1)%h) !!!!! a enlever
...
!!$       call second_degre(ha1 , ha2 , sqrt(g_r__8) ,sqrt(g_r__8) , -Tab(0)%qx)
!!$       if (ha1 >= 0.0d0) then
!!$          Ksi = ha1
!!$       else
!!$          Ksi = ha2
...
!!$       Tab(Nbx+2)%h = 1.0d0 !1.0d0 + 1.0d-01*(cos((8/(1.0d0*5.0d-02))*atan(1.0d0)*(time))) !2.0d0
!!$       H0 = 1.0d0
!!$       Ksi =  Tab(Nbx+2)%h - H0
!!$       
!!$       u_0 = Tab(Nbx+1)%qx/Tab(Nbx+1)%h
!!$       c_0 = SQRT(g_r__8*Tab(Nbx+1)%h)
!!$
!!$       u_1 = Tab(Nbx+2)%qx/Tab(Nbx+2)%h
!!$       c_1 = SQRT(g_r__8*Tab(Nbx+2)%h)
!!$       CALL CL_Subcritique_2Invariants(ksi, H0, u_0, u_1, c_0, c_1, Run%Dt, Dx, u, D)
!!$       Tab(Nbx+2)%qx = Tab(Nbx+2)%h*u
!!$       Tab(Nbx+1) =  Tab(Nbx+2)
!!$       Tab(Nbx) =  Tab(Nbx+1)

       Tab(Nbx+1)%h = 1.0d0 + 1.0d-01*(cos((8/(1.0d0*5.0d-02))*atan(1.0d0)*(time))) !1.0d0
...
!!!!!U(Nbx+1)%h = newton_gauche(U(Nbx+1)%h , U(Nbx+1)%uu , interpolation(x_R,te,yy,N))
