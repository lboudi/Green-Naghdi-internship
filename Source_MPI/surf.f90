PROGRAM SURF1D
  ...
  if(Me == 0) then
     ...
     CALL Initialise(i_case, DATA, Run , Me)
     ...
     N0 = 0
     CALL OUTPUT(N0, MESH, Var, DATA, Run, 'SOL1D_proc0.')
  ...
  loop__main : DO Niter = 1, DATA%NtMax !-- [MAIN TIME EVLUTION LOOP -- BEGIN] 
     ...
     IF(Run%time >= DATA%TimeOut) THEN
        ...
        CALL OUTPUT(Niter, MESH, Var, DATA, Run, 'SOL1D_proc0.')
  ...
  CALL CLOSE_ALL(DATA)
   ...
   if(Me == 10) then
     ...
     N0 = 0
     CALL OUTPUT(N0, MESH, Var, DATA, Run, 'SOL1D_proc1.')
  ...
  CALL CPU_TIME(Timeloop__1)
  DO Niter = 1, DATA%NtMax !-- [MAIN TIME EVLUTION LOOP -- BEGIN] 
     ...
     IF(Run%time >= DATA%TimeOut) THEN
        ...
        CALL OUTPUT(Niter, MESH, Var, DATA, Run, 'SOL1D_proc1.')
  ...
  END DO  ! Time evolution loop -- END --
