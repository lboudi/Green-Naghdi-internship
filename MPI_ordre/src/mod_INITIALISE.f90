...
!!$          end if
!!$
!!$          if ( MESH%X_DF(i) < 0.0d0 ) then
!!$             V%topoVF(i) = -1.25d0 !-10.0d0*(MESH%X_DF(i) - 0.4d0)*(MESH%X_DF(i)-0.6d0) !0.2d0 - 0.1d0*((x - 5.0d-01)**2)
!!$          else
!!$             V%topoVF(i) = -0.75d0
!!$          end if
          ...
          if(Me == 0) then
             ...
             V%topoVF(i) =  -1.0d0 !+ (1.0d0*tanh(1.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0) !-1.0d0

             V%VF(i)%h = 1.0d0 + 0.3d0*exp(-((MESH%X_DF(i)+0.5d0)/1.0d-01)**2)!&
                  !+ 1.0d0 - (1.0d0*tanh(1.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0) !-1.0d0 !+ 1.0d0 - (1.0d0*tanh(1.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0) !1.0d0 !0.0d0 !2.0d0 !max(V%topoVF(i),0.15) 
             V%VF(i)%qx = 0.0D0
          else

             
             V%topoVF(i) =  -1.0d0 !+ (1.0d0*tanh(1.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0)

             V%VF(i)%h = 1.0d0 + 0.3d0*exp(-((MESH%X_DF(i)+0.5d0)/1.0d-01)**2)!&
                  !+ 1.0d0 - (1.0d0*tanh(1.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0) !+ 1.0d0 - (1.0d0*tanh(1.0d01*(MESH%X_DF(i) - 0.0d-01))/4.0d0) !1.0d0 !0.0d0 !2.0d0 !max(V%topoVF(i),0.15) 
             V%VF(i)%qx = 0.0D0
